(function() {
  ...
  var module = angular.module('loom_chart_directive', []);

  module.directive('loomChart',
      function(mapService, tableViewService, statisticsService, $timeout) {
        return {
          replace: true,
          templateUrl: 'charts/partials/chart.tpl.html',
          link: function(scope, element) {
            scope.template = window.TEMPLATE;
            if (window.TEMPLATE === 'chart') {
              runDirective();
            }
            $(window.document.body).bind('enable-chart', function(evt, enabled) {
              $timeout(function() {
                scope.template = 'chart';
                runDirective();
              });
            });
